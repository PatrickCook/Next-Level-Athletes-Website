...
function update_schedule($day, $name, $sets, $reps) {
  ...
  $query = "INSERT INTO $day (ExerciseName, Sets, Reps) VALUES ('$name', $sets, $reps)";
  $result = mysqli_query($conn, $query);

  //$query = "UPDATE ExerciseList SET sets='$sets', reps='$reps' WHERE name='$name'";
  //$result = mysqli_query($conn, $query);
  echo $query;
}
...
function query_muscle_info($muscles) {
    include 'db_connect.php';
    $query = "SELECT * FROM ExerciseList WHERE " . $muscles;
    $result = mysqli_query($conn, $query);
    // Check if query returned any
    if (mysqli_num_rows($result) > 0) {
        $types = array();
        while(($row = $result->fetch_assoc())) {
            $types[] = $row['ExerciseName'];
        }
        echo json_encode($types);
    }
}
